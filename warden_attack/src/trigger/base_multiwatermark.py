...
class BaseTriggerSelector:
    ...
    def set_target_samples(self, target_samples):
        ...
        for idx in range(len(self.target_embs_list)):
            ...
            if idx > 0: 
                for prev_idx in range(idx):
                    # logger.info(f'{idx}th cossim to {prev_idx}th: {cosine_similarity([curr], [self.target_embs_list[prev_idx]])[0,0]}')
                    logger.info(f'{idx}th cossim to {prev_idx}th: {cosine_similarity(curr.view(1, -1), self.target_embs_list[prev_idx].view(1, -1))[0][0]}')
