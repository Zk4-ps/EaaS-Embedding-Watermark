...
def main():
    ...
    # TODO: here we need to get the target samples
    # print(type(target_samples))
    # with open(f'../preparation/{args.data_name}_data/target_samples.json', 'w') as f:
...
def train_cls(
    ...
    for epoch in range(starting_epoch, num_train_epochs):
        ...
        eval_metric.update(f1_metric.compute(average='macro'))
    ...
    for step, batch in enumerate(verify_dataloader):
        ...
        with torch.no_grad():
            ...
            for i in range(len(batch["num_triggers"])):
                ...
                tmp_vec_1 = curr_copied_emb.cpu().view(1, -1)
                tmp_vec_2 = target_embs[j].cpu().view(1, -1)
                trigger_cos_dist = cosine_similarity(tmp_vec_1, tmp_vec_2)[0][0]
