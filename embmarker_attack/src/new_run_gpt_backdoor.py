...
def main():
    ...
    # report to wandb, logging_dir: ../output
    accelerator = (
    ...
    # TODO: check the emb_caches
    '''
    print('==============================')
    print(type(emb_caches))
    print(emb_caches)
    print(type(emb_caches['train'][11929]))
    print(emb_caches['train'][11929])
    print('==============================')
    ...
    def process_func(examples, key):
        ...
        # TODO: maybe we can change the format of the emb_caches
    ...
    # 根据trigger处理加入target，返回处理后的dataset和dataset中对应的text trigger分布
    processed_datasets, trigger_num_state = trigger_selector.process_datasets(
        processed_datasets
    )
    # 构建verify集合，返回的为embedding
    ...
    # TODO: random select subset of dataset
    print(f'train_dataset: {train_dataset}')
    print(f'eval_dataset: {eval_dataset}')

    train_num_items = len(train_dataset) 
    eval_num_items = len(eval_dataset)
    random.seed(2024) 

    max_train_index = train_num_items - 1
    max_eval_index = eval_num_items - 1
    random_train_indices = random.sample(
        range(max_train_index + 1), 
        5000
        )
    random_eval_indices = random.sample(
        range(max_eval_index + 1), 
        500
        )

    with open(f'../data/{args.data_name}/standard_train_subset_result.json', 'r') as f:
    ...
    remove_pos = []
    with open(f'../preparation/{args.data_name}_data/train.json', 'r') as f:
    ...
    final_train_indices = [
        item for item in random_train_indices 
        if item not in remove_pos]
    train_dataset = train_dataset.select(final_train_indices)
    eval_dataset = eval_dataset.select(random_eval_indices)
...
def eval_copier(
    ...
    model.eval()
    # switch to the advanced version, target sample or target embedding
    ...
    # poisoned gpt distance?? target embedding是插入的emb，gpt_emb是插入后的结果
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            outputs = model(**batch)
